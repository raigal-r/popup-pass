...
const WorldIDAuth = ({ onSuccess }: { onSuccess?: () => void }) => {
  ...
  const handleMintPassport = () => {
    ...
    notification.success("Initiating passport minting...");
  };
  ...
  const VerificationContent = () => (
    <>
      ...
      <div className="space-y-4 w-full max-w-sm">
        ...
        <button className="btn btn-outline w-full" onClick={() => notification.info("Wallet connection coming soon!")}>
  ...
  return (
    <div className="card bg-base-100 shadow-xl">
      <div className="card-body items-center text-center">
        {!isVerified ? <VerificationContent /> : <VerificationSuccess method="World ID" />}
      </div>
    </div>
  );
};
